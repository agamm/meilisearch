...
pub(crate) mod test {
    ...
    fn import_dump_v5() {
        ...
        meili_snap::snapshot_hash!(meili_snap::json_string!(tasks), @"3c3ec8e5c66f00edd07a69073ac9adf1");
    ...
    fn import_dump_v4() {
        ...
        let tasks = dump.tasks().unwrap().collect::<Result<Vec<_>>>().unwrap();
        let (tasks, update_files): (Vec<_>, Vec<_>) = tasks.into_iter().unzip();
        meili_snap::snapshot_hash!(meili_snap::json_string!(tasks), @"3aea68654fdd615a895f4eb37b6c9914");
    ...
    fn import_dump_v3() {
        ...
        let tasks = dump.tasks().unwrap().collect::<Result<Vec<_>>>().unwrap();
        let (tasks, update_files): (Vec<_>, Vec<_>) = tasks.into_iter().unzip();
        meili_snap::snapshot_hash!(meili_snap::json_string!(tasks), @"4e2c5d10567ebb2e6394f51448bac92c");
        assert_eq!(update_files.len(), 10);
        assert!(update_files[0].is_some()); // the enqueued document addition
        assert!(update_files[1..].iter().all(|u| u.is_none())); // everything already processed
    ...
    fn import_dump_v2() {
        ...
        let tasks = dump.tasks().unwrap().collect::<Result<Vec<_>>>().unwrap();
        let (tasks, update_files): (Vec<_>, Vec<_>) = tasks.into_iter().unzip();
        meili_snap::snapshot_hash!(meili_snap::json_string!(tasks), @"3204b0ddc4eac38f8d8ca9303a58ffd4");
